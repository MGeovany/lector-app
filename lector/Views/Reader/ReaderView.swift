...
struct ReaderView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if viewModel.pages.isEmpty {
                viewModel.setPages(BookTextProvider.pages(for: book), initialPage: book.currentPage)
            }
        }
    }
    ...
    private var bottomPager: some View {
        HStack(spacing: 12) {
            Button {
                ...
                Image(systemName: "chevron.left")
                    .font(.system(size: 14, weight: .semibold))
                    .foregroundStyle(preferences.theme.surfaceText.opacity(viewModel.currentIndex > 0 ? 0.90 : 0.35))
            ...
            .accessibilityLabel("Page \(max(1, viewModel.currentIndex + 1)) of \(max(1, viewModel.pages.count))")
            ...
            Button {
                ...
                Image(systemName: "chevron.right")
                    .font(.system(size: 14, weight: .semibold))
                    .foregroundStyle(preferences.theme.surfaceText.opacity(viewModel.currentIndex < max(0, viewModel.pages.count - 1) ? 0.90 : 0.35))
                    .frame(width: 44, height: 44)
                    .background(
                        RoundedRectangle(cornerRadius: 14, style: .continuous)
                            .fill(preferences.theme.surfaceText.opacity(0.06))
    ...
    private var estimatedReadMinutes: Int {
        let wordCount = BookTextProvider.fullText(for: book).split { $0.isWhitespace || $0.isNewline }.count
        let wpm = 220.0
