...
struct ReaderControlsSheetView: View {
    ...
    private func themeChip(_ theme: ReadingTheme, title: String) -> some View {
        ...
        return Button {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        isSelected ? preferences.theme.accent.opacity(0.35) : preferences.theme.surfaceText.opacity(0.08),
                        lineWidth: 1
                    )
            )
        }
    ...
    private func stepperCapsule(title: String, value: String, onMinus: @escaping () -> Void, onPlus: @escaping () -> Void) -> some View {
        HStack(spacing: 10) {
